...
<nav class="navbar navbar-expand-md bg-light navbar-light border-bottom border-pr mb-2">
    ...
    <div class="collapse navbar-collapse" id="testnavbar1">
        <ul class="nav nav-pills my-lg-0 my-2">
            ...
            <li class="nav-item px-2">
                <a class="nav-link active" href="news.php">Новости <span class="sr-only">(current)</span></a>
            </li>
        </ul>
        <?php
        session_start();
...
$pgend=$pg*5;
include "db.php";
$res=mysqli_query($db,"SELECT * FROM `article` WHERE `type` LIKE 'Новость' ORDER BY `Date` DESC LIMIT $pgstart,$pgend");
$res2=mysqli_query($db,"SELECT `id` FROM `article` WHERE `type` LIKE 'Новость' ORDER BY `Date` DESC");
$i=1;
