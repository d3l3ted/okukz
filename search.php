...
<nav class="navbar navbar-expand-md bg-light navbar-light border-bottom border-pr mb-2">
    ...
    <div class="collapse navbar-collapse" id="testnavbar1">
        <ul class="nav nav-pills my-lg-0 my-2">
            ...
            <li class="nav-item px-2">
                <a class="nav-link active" href="articles.php">Статьи <span class="sr-only">(current)</span></a>
            </li>
            <li class="nav-item px-2">
...
$pgend=$pg*5;
include "db.php";
$srch=mysqli_real_escape_string($db,$_GET['search_query']);
$res=mysqli_query($db,"SELECT * FROM `article` WHERE `Zagolovok` LIKE '%$srch%' ORDER BY `Date` DESC LIMIT $pgstart,$pgend");
$res2=mysqli_query($db,"SELECT `id` FROM `article` WHERE `Zagolovok` LIKE '%$srch%' ORDER BY `Date` DESC");
...
echo "</ul></nav>";}else{
    echo "<div class='my-5'><p class='h2 ml-5'>К сожалению, поиск по сайту не дал никаких результатов</p>
    <p class='text-muted ml-5'>Попробуйте изменить или сократить ваш запрос</p>
    </div>";
}
?>
    </div>
    ...
    <div class="d-none d-lg-block col-2 mx-auto rounded px-0">
        <div class="stickypos bg-secondary w-100 sidebar-block rounded pt-3 shadow">
            ...
            while($row = mysqli_fetch_array($res1)) {
                ...
                <div class=\"col-12 align-self-center text-truncate\"><a class=\"h5 text-decoration-none\" href=\"?id=$id\">$zag</a></div>
