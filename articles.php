...
$pgend=$pg*5;
include "db.php";
$res=mysqli_query($db,"SELECT * FROM `article` WHERE `type` LIKE 'Статья' ORDER BY `Date` DESC LIMIT $pgstart,$pgend");
$res2=mysqli_query($db,"SELECT `id` FROM `article` WHERE `type` LIKE 'Статья' ORDER BY `Date` DESC");
$i=1;
